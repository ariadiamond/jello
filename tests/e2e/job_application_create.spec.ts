...
test.describe('Job Application Creation', () => {
  ...
  test('Can create a job application', async ({ page }) => {
    ...
    const record = database.prepare('SELECT ja.* FROM job_applications ja INNER JOIN companies c ON c.id = ja.company_id WHERE ja.title = ? AND c.name = ?').get(jobTitle, companyId);
    await expect(page).toHaveURL(`http://localhost:${process.env.PORT}/job_application/${record.id}`);
  });
